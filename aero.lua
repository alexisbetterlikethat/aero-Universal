...
local TeleportDropdownPrimary = PlayerTeleportSection:CreateDropdown({
    ...
    CurrentOption = {"None"},
    Callback = function(val)
        local name = val[1] or val
        if name == "None" then
            selectedTeleportTarget = nil
        else
            selectedTeleportTarget = Players:FindFirstChild(name)
        end
    end
})
...
PlayerTeleportSection:CreateButton({
    ...
    Callback = function()
        ...
        local targetChar = selectedTeleportTarget.Character
        local targetRoot = targetChar and targetChar:FindFirstChild("HumanoidRootPart")
        
        if not targetRoot then
            return Luna:Notification({
                Title = "Teleport",
                Content = "Target has no character/root part.",
                Icon = "error"
            })
        end
...
PlayerTeleportSection:CreateButton({
    ...
    Callback = function()
        ...
        if TeleportDropdownPrimary.UpdateOptions then
            TeleportDropdownPrimary:UpdateOptions(names)
        end
    end
})
